        ...
        <div class="pull-right">
            ...
            <div class="d-i_b">
                ...
                <button type="button" class="btn btn-small action_on formSubmit" data-action="close" data-form="#addCallbackStatusForm"><i class="icon-check"></i>{lang('Save and go back','admin')}</button>
                    {echo create_language_select($languages, $locale, '/admin/components/run/shop/callbacks/updateStatus/'. $model->getId())}
            </div>
        </div>
    </div>
...
{if count($languages) > 1}
    <span style="float:right;">
        {lang('Language','admin')}:
        <select name="Language" onchange="javascript:ajaxShop('callbacks/updateStatus/{echo $model->getId()}/' + this.value);
                return false;">
            {foreach $languages as $language}
                <option value="{echo $language.identif}" {if $language.identif == $locale}selected{/if}>{echo $language.lang_name}</option>
            {/foreach}
        </select>
        <img width="16" height="16" align="absmiddle" src="/application/modules/shop/admin/templates/assets/images/translateable.png" title="{lang('This icon denotes a field that can be translate well another language.','admin')}">
    </span>
{/if}
{*/}