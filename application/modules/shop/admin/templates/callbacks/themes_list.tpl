        ...
        <section class="mini-layout">
            <div class="frame_title clearfix">
                ...
                <div class="pull-right">
                    <div class="d-i_b">
                        <a  href="{$ADMIN_URL}callbacks/createTheme" class="pjax btn btn-small btn-success"><i class="icon-plus-sign icon-white"></i>{lang('Create topic','admin')}</a>
                    </div>
                </div>
            </div>
            <div class="tab-content">
                <div class="row-fluid">
                    <table class="table  table-bordered table-hover table-condensed t-l_a">
                        ...
                        <tbody class="sortable save_positions" data-url="/admin/components/run/shop/callbacks/reorderThemes">
                            {foreach $model as $c}
                                <tr data-id="{echo $c->getId()}">
                                    ...
                                    <td>{echo $c->getId()}</td>
                                    <td>
                                        <a href="{$ADMIN_URL}callbacks/updateTheme/{echo $c->getId()}/{$locale}" data-rel="tooltip" data-title="{lang('Edit callback theme','admin')}">{echo ShopCore::encode($c->getText())}</a>
                                    </td>
            ...
            <div class="alert alert-info" style="margin-bottom: 18px; margin-top: 18px;">
                {lang('List of callbacks is empty','admin')}
            </div>
            {/if*/}
            </section>
        </form>
