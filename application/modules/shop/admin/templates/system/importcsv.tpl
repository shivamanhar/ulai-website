...
<div class="saPageHeader">
    <h2>{lang('Products import','admin')}</h2>
</div>
...
<form method="post" action="{$ADMIN_URL}system/import"  enctype="multipart/form-data" style="width:100%" id="file_upload_form">
    <div style="float:left;width:500px;">
        <div class="form_text">{lang('File','admin')}:</div>
        <div class="form_input">
            <input type="file" name="userfile" />
        </div>
        <div class="form_overflow"></div>

        <div class="form_text">{lang('Import type','admin')}:</div>
        <div class="form_input">
            <select name="import_type"  style="width:310px;">
                <option value="ProductsImport">{lang('Products','admin')}</option>
                <option value="CategoryImport">{lang('Catalog','admin')}</option>
                <option value="import_properties">{lang('Properties','admin')}</option>
            </select>
        </div>
        <div class="form_overflow"></div>
        ...
        <div class="form_input">
            <input type="text" name="attributes" id="attributesBox" value="cat, num, name, prc" class="textbox_long" style="width:300px;"/>
        </div>
        <div class="form_overflow"></div>

        <div class="form_text">{lang('The field separator','admin')}:</div>
        <div class="form_input">
            <input type="text" class="textbox_long" style="width:24px;" value=";" name="delimiter" id="delimiterText"/>
            <select onchange="$('delimiterText').set('value', this.value)" style="width:273px;">
        ...
        <div class="form_input">
            ...
            <select onchange="$('enclosureText').set('value', this.value)" style="width:273px;">
        ...
        <div class="form_input">
            <select name="encoding" style="width:310px;">
        ...
        <div class="form_input">
            <select name="language" style="width:310px;">
        ...
        <div class="form_text">{lang('Currency','admin')}:</div>
        <div class="form_input">
            <select name="currency">
                {foreach $currencies as $currency}
                    <option value="{echo $currency->id}">{echo $currency->name} ({echo $currency->code})</option>
                {/foreach}
            </select>
        </div>
        <div class="form_overflow"></div>        
        ...
        <div class="form_input">
            <input type="submit" value="{lang('Import','admin')}" class="button_130"/>
        </div>
        <div class="form_overflow"></div>
    </div>
    ...
    <div style="float:left;">
        ...
        <div class="form_input">
            <table cellpadding="1" cellspacing="3" style="font-size:11px" class="attributesTable">
                <tr class="noHover"><td><b>{lang('Columns','admin')}</b></td><td style="width:250px;"></td></tr>
    ...
    <script type="text/javascript">
                ...
                function fileUploadCallback()
                {
                    ...
                    if (data != '')
                    {
                        ...
                        $('errorsBox').set('html', '');
                        showMessage('', lang('Import completed', 'admin'));
                    }
                }
    </script>
{/literal}
