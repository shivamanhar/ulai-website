...
class Admin extends BaseAdminController {
    ...
    function create_item($id = null) {
        if (empty($_POST)) {
            ...
            if ($this->form_validation->run($this) == FALSE) {
                ...
                $last_item_position = $this->db->where('menu_id', $_POST['menu_id'])
                ...
                if (!isset($item_data['add_data'])) {
					...
					$data['page_parent'] = (int)$_POST['page_parent']; 
    ...
    public function loadPathImg() {
        $pathUrl = $this->db->like('add_data', $_POST['title'])->where('item_type', 'module')->get('menus_data')->row()->item_image;
        if ($pathUrl) {
            echo $pathUrl;
        }
    }
    ...
    function edit_item($item_id) {
        if (empty($_POST)) {
			...
			$page_parent=isset($data['page_parent'])?$data['page_parent']:0;			
			$per_page=isset($data['per_page'])?$data['per_page']:10;		
            $pages = $this->get_pages($page_parent, 0, 'controller',$per_page);
            $locale = MY_Controller::getCurrentLocale();
            $this->db->select("shop_rbac_roles.*", FALSE);
            $this->db->select("shop_rbac_roles_i18n.alt_name", FALSE);
            $this->db->where('locale', $locale);
            ...
            if ($this->form_validation->run($this) == FALSE) {
                ...
                if ($item_data['item_type'] == 'page') {
                    $item_data['add_data'] = serialize(array('page' => $_POST['item_url'], 'page_parent' => $_POST['page_parent'], 'per_page' => (int)$_POST['per_page'], 'newpage' => (int) $newpage));
                }
                if (!isset($item_data['add_data']))
                    $item_data['add_data'] = serialize($data);
                $errorMessage = Null;
...
//            $html .= $item['title'];
    ...
    function insert_menu_item() {
        ...
        if (!isset($item_data['add_data']))
            $item_data['add_data'] = serialize(array('newpage' => $_POST['newpage'], 'page_parent' => (int)$_POST['page_parent'], 'per_page' => (int)$_POST['per_page']));
...
//        $val->set_rules('menu_tpl', lang("Template folder", 'menu'), 'required|max_length[5000]');
     ...
     */
    function get_pages($cat_id = 0, $cur_page = 0, $referer = null, $per_page=10) {
        ...
        if ($pages->num_rows() > 0) {
            ...
            $total = $this->db->get_where('content', array('lang' => $this->default_lang_id, 'category' => $cat_id))->num_rows();
