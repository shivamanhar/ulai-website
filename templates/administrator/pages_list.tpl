...
<div class="modal hide fade" id="pages_action_dialog_copy">
    ...
    <div class="modal-body">
        {lang("Category","admin")}:
        <select id="CopyMoveCategorySelect2" name="CopyMoveCategorySelect" url="{$BASE_URL}admin/pages/GetPagesByCategory/">
            <option value="0">{lang("Without a category","admin")}</option>
            {$this->view("cats_select.tpl", array('tree' => $this->template_vars['tree'] ));}
        </select>
    </div>
    ...
    <section class="mini-layout">
        <div class="frame_title clearfix">
            <div class="pull-left">
                ...
                <span class="title">{lang('Articles list', 'admin')} ({echo $total_pages})</span>
            </div>
            <div class="pull-right">
                <div class="d-i_b">
                    ...
                    pagesAdmin.updDialogCopy();" type="button" class="btn btn-small btn-danger disabled action_on pages_action pages_delete" ><i class="icon-trash"></i>{lang('Delete','admin')}</button>
                    ...
                    <a class="btn btn-small btn-success" href='{$BASE_URL}admin/pages'><i class="icon-plus-sign icon-white"></i>{lang('Create page','admin')}</a>
                </div>
            </div>
        </div>
        <div class="row-fluid">
                ...
                <div class="inside_padd">
                    <ul class="nav nav-tabs nav-stacked">
                        ...
                        <li {if $cat_id==$sc1.id} class="active" {/if}>
                            <a  href="/admin/pages/GetPagesByCategory/{$sc1.id}" class="pjax">&nbsp;&nbsp;&nbsp;{$sc1.name} </a>
                        </li>
                        {if $sc1.subtree}
                        {foreach $sc1.subtree as $sc2}
                        <li {if $cat_id==$sc2.id} class="active" {/if}>
                            <a  href="/admin/pages/GetPagesByCategory/{$sc2.id}" class="pjax">&nbsp;&nbsp;&nbsp;&nbsp;{$sc2.name} </a>
                        </li>
                        {if $sc2.subtree}
                        {foreach $sc2.subtree as $sc3}
                        <li {if $cat_id==$sc3.id} class="active" {/if}>
                            <a  href="/admin/pages/GetPagesByCategory/{$sc3.id}" class="pjax">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{$sc3.name} </a>
                        </li>
                        {if $sc3.subtree}
                        {foreach $sc3.subtree as $sc4}
                        <li {if $cat_id==$sc4.id} class="active" {/if}>
                            <a  href="/admin/pages/GetPagesByCategory/{$sc4.id}" class="pjax">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{$sc4.name} </a>
                        </li>
                        {/foreach}
                        {/if}
                        {/foreach}
            ...
            <div class="span9">
                <div class="inside_padd">
                    <table class="table  table-bordered table-hover table-condensed pages-table t-l_a" {if $show_cat_list != 'yes'} style="width:100%;"{/if}>
                        <thead>
                            <tr>
                                ...
                                <th class="span3">{lang('Title','admin')}</th>
