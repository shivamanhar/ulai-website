                ...
                <div class="frame-button-add-wish-list">
                    <div class="btn-cart">
                        <button type="button" data-drop=".drop-add-wishlist" data-place="inherit" data-overlay-opacity="0" data-effect-on="slideDown" data-effect-off="slideUp">
                            ...
                            <span class="text-el">{lang('Создать новый список','light')}</span>
                        </button>
                    </div>
                    <span class="help-block">{lang('В список избранных вы можете отложить понравившиеся товары, также показать список друзьям', 'light')}</span>
                </div>
                <div class="drop drop-style-2 drop-add-wishlist">
                    <div class="drop-header">
                        <div class="title">{lang('Создание списка избранных товаров','light')}</div>
                    </div>
                            ...
                            <div class="horizontal-form big-title">
                                <form method="POST" action="{site_url('/wishlist/wishlistApi/createWishList')}">
                                    ...
                                    <div class="frame-label">
                                        <span class="title">{lang('Доступность:','light')}</span>
                                        <div class="frame-form-field check-public">
                                            <div class="lineForm">
                                                <select name="wlTypes" id="wlTypes">
                                                    <option value="shared">{lang('Коллективный', 'light')}</option>
                                                    <option value="public">{lang('Публичный', 'light')}</option>
                                                    <option value="private">{lang('Приватный', 'light')}</option>
                                                </select>
                                            </div>
                                        </div>
                                    </div>
                                    <label>
                                        <span class="title">{lang('Название списка:','light')}</span>
                                        <span class="frame-form-field">
                                            <input type="text" value="" name="wishListName"/>
                                        </span>
                                    </label>
                                    <label>
                                        <span class="title">{lang('Описание:','light')}</span>
                                        <span class="frame-form-field">
                ...
                <div class="inside-padd">
                    <div class="funcs-buttons-wishlist d_i-b">
                        <div class="btn-edit-WL">
                            ...
                            data-always="true"
                            >
                            <span class="d_l_1 text-el">{lang('Редактировать список','light')}</span>
                        </button>
                    </div>
                    <div class="btn-remove-WL">
                        ...
                        <span class="text-el d_l_1">{lang('Удалить список','light')}</span>
                    </button>
                </div>
            </div>
            <div class="d_i-b m-r_10">
                <b>{lang('Доступность:','light')}</b>
                {if $wishlist[0][access] == 'private'}
                <span class="s_t">{lang('Приватный', 'light')}</span>
                {/if}
                {if $wishlist[0][access] == 'public'}
                <span class="s_t">{lang('Публичный', 'light')}</span>
                {/if}
                {if $wishlist[0][access] == 'shared'}
                <span class="s_t">{lang('Коллективный', 'light')}</span>
                {/if}
            </div>
            {if $wishlist[0]['access'] == 'shared' || $wishlist[0]['access'] == 'public'}
            <div class="btn-form btn-send-wishlist d_i-b m-r_10">
                <button type="button" data-drop=".drop-sendemail" title="{lang('Поделится с другом','light')}" data-source="{echo site_url('wishlist/wishlistApi/renderEmail/' . $wishlist[0][wish_list_id])}">
