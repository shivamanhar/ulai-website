...
{else:}
    ...
    <div class="content_head">
        <div class="crumbs">
            <h1>{lang('Вы искали','commerce_mobiles')}: "{encode($_GET['text'])}"</h1>
        </div>
    </div>
    {if $totalProducts > 0}
        <ul class="catalog">
...
<span class="d_b price">{echo \Currency\Currency::create()->getCurrencyToFormat(\Currency\Currency::create()->getMainCurrency()->getId(), $p->firstVariant->toCurrency(),'', '', '',  '', '', '');}</span> 
                        </span>
                    </a>
                </li>
            {/foreach}
