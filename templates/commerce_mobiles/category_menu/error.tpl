...
{/literal}
<div class="error">
    <h3 align="center">{lang('Внимание','commerce_mobiles')}!</h3>
    <p>{lang('Не обнаружен необходимый файл шаблона для меню','commerce_mobiles')} <b>{$menu}</b></p>
    <ul>
        {foreach $errors as $e}
    ...
    <p>
        {if $tpl_folder}
            {lang('Проверьте наличие файла или измените настройки меню','commerce_mobiles')} <b>{$menu}</b>
        {else:}
            {lang('Переустановите модуль меню','commerce_mobiles')}
        {/if}
    </p>
</div>	